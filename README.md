...
Paste a link to a song, playlist, or album from YouTube, YouTube Music, or Spotify and the download starts. There's no browser tab to fuss with and no login. On top of downloading, SetEngine can listen to a recorded DJ set and tell you the tracklist, work out the BPM and key of your files without going online, suggest tracks that mix in key, and put a setlist in harmonic order. It's a crate manager, not just a downloader.
...
2. **Set Extraction** (beta). Paste a link to a recorded YouTube DJ set and it identifies the tracks that were played, lists them in the order they came in, and lets you download each one, or grab the whole set together with a playlist (`.m3u`) file.
3. **Set Maker.** Analyze a folder of your music for BPM and key (no internet needed), rate your tracks, and build a setlist that's ordered to mix in key. Import and export playlist files.
...
- **Set Extraction.** Point it at a recorded DJ set and it names the tracks one by one using AudD or ACRCloud (you bring your own account key), then you download them individually or grab the whole set with a playlist file.
...
3. **Set Extraction.** Paste a set link, let it work out the tracklist, then download individual tracks or the whole set plus a playlist file.
...
> **Heads up:** Set Extraction is still in beta. It names tracks by listening to the audio, which doesn't always get it right. Unreleased IDs, bootlegs, mashups, and heavily-edited tracks often can't be matched. Treat the tracklist as a starting point, not gospel.
